...
#      restrictions. Pass flags instead (see below) for a fully
#      non-interactive run.
...
#   4. Web app (only in --mode webapp): calls setup-new-machine.sh with the
#      translated flags below. That script is already fully idempotent on
#      its own (see its own header) -- setup.sh doesn't duplicate any of
#      its state checks, just forwards flags to it.
#   5. GeoIP (optional, only if requested): writes MAXMIND_LICENSE_KEY into
...
#   6. Writes a provisioning marker (see below) on first successful run,
