...
#
# This USED to `sed -i` the mount line straight into the tracked
# docker-compose.yml. That permanently diverged every host that ran this
# script from git -- a modified tracked file that `git pull`/upgrade.sh's
# clean-tree preflight check refuses to fast-forward over, forever, on
# every single host-executor-enabled machine. Reproduced live on the test
# box 2026-08-21 ("please fix this git pull issue on test box
# permanently").
#
# Fixed structurally: write the mount into docker-compose.override.yml
# instead (already gitignored -- see .gitignore -- `docker compose`
# auto-merges it with docker-compose.yml, no CLI flag needed). The tracked
# file itself is never touched, on any host, so it always matches origin
# exactly and `git pull` never has anything local to conflict with.
enable_deploy_key_mount() {
	log "Phase 4: enable deploy-key volume mount via docker-compose.override.yml"
	local override_file="$REPO_DIR/docker-compose.override.yml"
	if [[ -f "$override_file" ]] && grep -qE '^\s*-\s*\$\{HOST_SSH_KEY_SOURCE_PATH' "$override_file"; then
		log "  already enabled."
		return
	fi
	cat > "$override_file" <<-'EOF'
	# Host-specific override, generated by setup-new-machine.sh -- gitignored
	# (see .gitignore), never committed, never touched by `git pull`. Docker
	# Compose auto-merges this with docker-compose.yml on every `up`/`config`
