...
- **Cities:** one per line, free text, matched case-insensitively as an exact name (e.g. `Karachi`) — there's no fixed enumerable list of world cities the way there is for country codes, and GeoLite2's city-level accuracy is coarser than country, so treat this as a useful signal rather than a precise location.
- **Networks (ASN):** one per line, either form works (`AS15169` or `15169`) — identifies the network/ISP a connection came from rather than a physical location, e.g. to always allow a known corporate network's ASN regardless of which country its NAT gateway happens to geolocate to.
