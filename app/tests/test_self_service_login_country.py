...
class TestSelfServiceLoginCountry:
    def test_set_country_writes_only_to_vpn_policy_not_portal_columns(self, app_client, db_session, monkeypatch, tmp_path, default_group_id):
        ...
        app_client.post("/api/users", json={
            ...
            "email": "geouser@example.com", "mac": "aa:bb:cc:dd:ee:10", "group_id": default_group_id,
        })
    ...
    def test_clearing_country_removes_vpn_restriction(self, app_client, monkeypatch, tmp_path, default_group_id):
        from vpnadmin.routes import users as users_mod
        monkeypatch.setattr(users_mod.cli, "add_client", lambda name, mac: f"{name} added.")
        monkeypatch.setattr(settings, "CLIENT_POLICY_FILE", str(tmp_path / "client_policy.json"))
        login(app_client, "admin", "adminpass123")
        app_client.post("/api/users", json={
            ...
            "email": "geouser2@example.com", "mac": "aa:bb:cc:dd:ee:11", "group_id": default_group_id,
        })
        app_client.post("/logout")
    ...
    def test_omitted_field_leaves_vpn_restriction_untouched(self, app_client, monkeypatch, tmp_path, default_group_id):
        from vpnadmin.routes import users as users_mod
        monkeypatch.setattr(users_mod.cli, "add_client", lambda name, mac: f"{name} added.")
        monkeypatch.setattr(settings, "CLIENT_POLICY_FILE", str(tmp_path / "client_policy.json"))
        login(app_client, "admin", "adminpass123")
        app_client.post("/api/users", json={
            ...
            "email": "geouser4@example.com", "mac": "aa:bb:cc:dd:ee:13", "group_id": default_group_id,
        })
        app_client.post("/logout")
    ...
    def test_setting_vpn_country_does_not_block_own_portal_login(self, app_client, monkeypatch, tmp_path, default_group_id):
        ...
        app_client.post("/api/users", json={
            ...
            "email": "geouser5@example.com", "mac": "aa:bb:cc:dd:ee:14", "group_id": default_group_id,
        })
        app_client.post("/logout")
    ...
    def test_admin_can_clear_vpn_restriction_via_manage_restrictions_api(self, app_client, monkeypatch, tmp_path, default_group_id):
        from vpnadmin.routes import users as users_mod
        monkeypatch.setattr(users_mod.cli, "add_client", lambda name, mac: f"{name} added.")
        monkeypatch.setattr(settings, "CLIENT_POLICY_FILE", str(tmp_path / "client_policy.json"))
        login(app_client, "admin", "adminpass123")
        app_client.post("/api/users", json={
            ...
            "email": "geouser6@example.com", "mac": "aa:bb:cc:dd:ee:15", "group_id": default_group_id,
        })
        app_client.post("/logout")
