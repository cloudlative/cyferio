...
class TestPasswordComplexityPolicy:
    def test_create_user_rejects_password_missing_uppercase(self, app_client, monkeypatch):
        ...
        r = app_client.post("/api/users", json={
            "username": "weakpwuser", "password": "lowercase123!", "first_name": "Weak",
            "email": "weakpwuser@example.com", "mac": "aa:bb:cc:dd:ee:01",
        })
    ...
    def test_create_user_rejects_password_missing_special_char(self, app_client, monkeypatch):
        ...
        login(app_client, "admin", "adminpass123")
        r = app_client.post("/api/users", json={
            "username": "nospecialuser", "password": "Lowercase123", "first_name": "NoSpecial",
            "email": "nospecialuser@example.com", "mac": "aa:bb:cc:dd:ee:02",
        })
        assert r.status_code == 422
    ...
    def test_create_user_rejects_password_missing_digit(self, app_client, monkeypatch):
        ...
        login(app_client, "admin", "adminpass123")
        r = app_client.post("/api/users", json={
            "username": "nodigituser", "password": "Nodigitpass!", "first_name": "NoDigit",
            "email": "nodigituser@example.com", "mac": "aa:bb:cc:dd:ee:03",
        })
        assert r.status_code == 422
    ...
    def test_self_service_password_change_rejects_noncompliant_password(self, app_client):
        ...
        r = app_client.patch("/api/users/me", json={
            "current_password": "viewerpass123", "new_password": "allweak",
        })
        assert r.status_code == 422
...
class TestMacFormatValidation:
    def test_create_user_rejects_malformed_mac(self, app_client, monkeypatch):
        ...
        login(app_client, "admin", "adminpass123")
        r = app_client.post("/api/users", json={
            "username": "badmacuser", "password": "Goodpass123!", "first_name": "BadMac",
            "email": "badmacuser@example.com", "mac": "not-a-mac-address",
        })
        assert r.status_code == 422
